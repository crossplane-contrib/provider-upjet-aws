...
func SetupWebhookWithManager(mgr ctrl.Manager) error {
	if err := ctrl.NewWebhookManagedBy(mgr, &v1beta1.PrimaryContact{}).
		Complete(); err != nil {
		return errors.Wrap(err, "cannot register webhook for the kind v1beta1.PrimaryContact")
	}
	return nil
}

// SetupGated adds a controller that reconciles PrimaryContact managed resources.
func SetupGated(mgr ctrl.Manager, o tjcontroller.Options) error {
	o.Options.Gate.Register(func() {
		if err := Setup(mgr, o); err != nil {
			mgr.GetLogger().Error(err, "unable to setup reconciler", "gvk", v1beta1.PrimaryContact_GroupVersionKind.String())
		}
	}, v1beta1.PrimaryContact_GroupVersionKind)
	return nil
}

// Setup adds a controller that reconciles PrimaryContact managed resources.
func Setup(mgr ctrl.Manager, o tjcontroller.Options) error {
	name := managed.ControllerName(v1beta1.PrimaryContact_GroupVersionKind.String())
	var initializers managed.InitializerChain
	rl := reconciliationpolicy.NewExponentialFailureRateLimiter(time.Second, 60*time.Second)
	eventHandler := handler.NewEventHandler(handler.WithDefaultRateLimiter(rl), handler.WithLogger(o.Logger.WithValues("gvk", v1beta1.PrimaryContact_GroupVersionKind)))
	...
	if o.MetricOptions != nil && o.MetricOptions.MRStateMetrics != nil {
		stateMetricsRecorder := statemetrics.NewMRStateRecorder(
			mgr.GetClient(), o.Logger, o.MetricOptions.MRStateMetrics, &v1beta1.PrimaryContactList{}, o.MetricOptions.PollStateMetricInterval,
		)
		if err := mgr.Add(stateMetricsRecorder); err != nil {
			return errors.Wrap(err, "cannot register MR state metrics recorder for kind v1beta1.PrimaryContactList")
		}
	}

	r := managed.NewReconciler(mgr, xpresource.ManagedKind(v1beta1.PrimaryContact_GroupVersionKind), opts...)
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		Watches(&v1beta1.PrimaryContact{}, eventHandler).
		Complete(
