...
func ExternalNameConfigurations() config.ResourceOption {
	return func(r *config.Resource) {
		if e, ok := ExternalNameConfigs[r.Name]; ok {
			...
			r.ExternalName = e
			// Note(turkenh): This is special to provider-aws and would not be
			// needed for other providers. We had injected region as a parameter
			// for all resources to be consistent with native aws provider, and
			// now, we need to add manually it to the identifier fields.
