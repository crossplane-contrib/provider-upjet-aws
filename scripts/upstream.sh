...
DESCRIPTION
  ...
  This script is copied from the pulumi-aws repository. Its usage in crossplane
  provider-upjet-aws is still experimental.
EXAMPLES
...
assert_not_checked_out() {
  ...
  if [[ "${current_branch}" == "patcher-tool/patch-checkout" ]]; then
    cat <<EOF
Error: 'upstream' submodule checked out on the 'patcher-tool/patch-checkout' branch.
This was likely caused by running a 'checkout' command but not running
'check_in' afterwards.

To turn the commits in the 'patcher-tool/patch-checkout' branch back into patches, run:
  ${original_exec} check_in

To disgard changes in the 'patcher-tool/patch-checkout' branch, use the 'force' flag (-f):
...
assert_is_checked_out() {
  current_branch=$(cd upstream && git --no-pager rev-parse --abbrev-ref HEAD)
  if [[ "${current_branch}" != "patcher-tool/patch-checkout" ]]; then
    echo "Expected upstream to be checked out on the 'patcher-tool/patch-checkout' branch, but ${current_branch} is checked out."
    exit 1
  fi
}
...
clean_branches() {
  cd upstream
  if git show-ref --verify --quiet refs/heads/patcher-tool/patch-checkout; then
    git branch -D patcher-tool/patch-checkout
  fi
  if git show-ref --verify --quiet refs/heads/patcher-tool/checkout-base; then
    git branch -D patcher-tool/checkout-base
  fi
  if git show-ref --verify --quiet refs/heads/patcher-tool/original-base; then
    git branch -D patcher-tool/original-base
  fi
  cd ..
}
...
checkout() {
  ...
  # Set the 'patcher-tool/checkout-base' branch to the current commit of the upstream repository
  # This is used to track the base commit of the patches
  # If rebasing, then this must be moved to the new base commit.
  git branch -f patcher-tool/checkout-base
  # Create a new branch 'patcher-tool/patch-checkout' which will contain the commits for each patch
  git checkout -B patcher-tool/patch-checkout
...
The 'patcher-tool/patch-checkout' branch is pointing to the last patch.
The 'patcher-tool/checkout-base' branch is pointing to the base commit of the patches.
...
rebase() {
  # Parse additional flags
  onto="patcher-tool/checkout-base"
  interactive="false"
  ...
  git fetch --all
  # Set the "patcher-tool/original-base" branch to the current base commit of the patches
  git branch -f patcher-tool/original-base patcher-tool/checkout-base
  # Set the "patcher-tool/patch-checkout" branch to track the "patcher-tool/original-base" branch
  git branch --set-upstream-to=patcher-tool/original-base patcher-tool/patch-checkout
  # Set the "patcher-tool/checkout-base" branch to the new base commit ready for formatting the patches after
  git branch -f patcher-tool/checkout-base "${onto}"
  # Rebase the 'patcher-tool/patch-checkout' branch on top of the new base commit
  interactive_flag=""
...
export_patches() {
  ...
  # Extract patches from the commits in the 'patcher-tool/patch-checkout' branch into the 'patches' directory.
  # Use the 'patcher-tool/checkout-base' branch to determine the base commit of the patches.
  (cd upstream && git format-patch patcher-tool/checkout-base -o ../patches --zero-commit --no-signature --no-stat --no-numbered)
}
...
check_in() {
  ...
  (cd upstream && git checkout patcher-tool/checkout-base)
