...
debug:
	@$(INFO) debug
	mkdir -p _output/controlplane-dump
	cp "${UPTEST_DATASOURCE_PATH}" _output/controlplane-dump
	@$(OK) debug
# This target requires the following environment variables to be set:
# - UPTEST_CLOUD_CREDENTIALS, cloud credentials for the provider being tested, e.g. export UPTEST_CLOUD_CREDENTIALS=$(cat ~/.aws/credentials)
# - UPTEST_EXAMPLE_LIST, a comma-separated list of examples to test
# - UPTEST_DATASOURCE_PATH, see https://github.com/upbound/uptest#injecting-dynamic-values-and-datasource
e2e: debug build controlplane.up local.xpkg.deploy.provider.$(PROJECT_NAME) uptest
